...
-----------------------------------------------------------------------------------------------------------------------------------------
vCLIENT = {}
Tunnel.bindInterface("arcade-ammunation",vCLIENT)
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIAVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
local machineGlobal = 1800 -- Tempo para poder assaltar um Ammu-Nation novamente
...
local pagamentoMin = 15000 -- Valor minimo que recebe ao assaltar o Ammu-Nation
local pagamentoMax = 25000 -- Valor maximo que recebe ao assaltar o Ammu-Nation
local tempoRoubo = 1800 -- Tempo para poder assaltar um Ammu-Nation novamente
local recompensa = "dinheirosujo" -- Item dropado quando o Ammu-Nation for explodido
-----------------------------------------------------------------------------------------------------------------------------------------
...
function vCLIENT.startVault()
	...
	if user_id then
		...
		elseif (os.time()-machineGlobal) <= tempoRoubo then
			TriggerClientEvent("Notify",source,"aviso","Os cofres estão vazios, aguarde <b>"..vRP.format(parseInt((tempoRoubo-(os.time()-machineGlobal)))).." segundos</b> até que os civis depositem dinheiro.",8000)
			return false
		else
	...
	for l,w in pairs(policia) do
		...
		if player then
			async(function()
				...
				blips[ids] = vRPC.addBlip(player,x,y,z,1,59,"Roubo de Ammu-Nation em andamento",0.5,true)
				TriggerClientEvent('chatMessage',player,"190",{64,64,255},"O roubo começou em uma ^1Ammu-Nation^0, dirija-se até o local e intercepte o assaltante.")
...
function vCLIENT.stopVault(x,y,z)
	...
	if user_id then
		if machineStart then
			...
			if porcentagem <= 30 then -- PORCENTAGEM DE CHANCE DE DROPAR A FIVE-SEVEN
			TriggerEvent("DropSystem:create","wbody|WEAPON_PISTOL_MK2",parseInt(1),x,y,z,3600)
			end
		end
	end
end
